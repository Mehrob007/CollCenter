...
export default function CreateTask(openCreate, idTask) {
    ...
    const [getDataTaskState, setGetDataTaskState] = useState();
    ...
    useEffect(() => {
        if (interactionIdContacts) {
            getDataTask(interactionIdContacts)
        }
    }, [interactionIdContacts])
    const getDataTask = async (dataGet) => {
        const token = localStorage.getItem('accessToken');
        try {
            const res = await apiClient.get(`api/interactions/all?pagination.limit=15&pagination.page=1&search=${dataGet}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
            console.log(res.data.interactions[0]);
            setGetDataTaskState(res.data.interactions[0])
            setFormData((prev) => ({
                ...prev,
                interactionId: res.data.interactions[0].id,
            }))
        } catch (error) {   
            console.log(error);
            if (error.response.status === 401) {
    ...
    return (
        <div className='WriteLetter'>
            ...
            <div className="headerWL">
                {!openCreate && <h1>Создать задачу</h1>}
